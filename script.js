// create your terminal and edit it your way // 
...
function Square(z) {
  ...
  this.points = [
    new Point({
      x: (canvas.width / 4) - this.width,
      y: (canvas.height / 4) - this.height,
      z: z
    }),
    new Point({
      x: (canvas.width / 4) + this.width,
...
function render() {
  ...
  for (i = 0; i < canvasBars.width; i += 40) {
    if (!barVals[i]) {
      barVals[i] = {
        val: Math.random() * (canvasBars.height / 4),
        freq: 0.2,
        sineVal: Math.random() * 200
      };
    }
...
setTimeout(function () {
  ...
  for (var i = 0; i < 15; i++) {
    squares.push(new Square(-600 + (i * 900)));
  }

  ctx.strokeStyle = ctxBars.strokeStyle = ctxBars.fillStyle = '#5534A5';
...
window.addEventListener('resize', function () {
  ...
  canvasBars.width = window.innerWidth / 4;
  canvasBars.height = canvas.height;

  outputConsole.style.height = (window.innerHeight / 6) * 4 + 'px';
  outputConsole.style.top = window.innerHeight / 6 + 'px';

  focal = canvas.width / 4;
  vpx = canvas.width / 4;
  vpy = canvas.height / 4;
  ctx.strokeStyle = ctxBars.strokeStyle = ctxBars.fillStyle = '#5534A5';
});
